...
export class ratasenlasparedesNpcSheet extends ActorSheet {
  ...
  async getData() {
    ...
    system.pv.value = Number.isNaN(Number(system.pv.value))
      ? 0
      : Number(system.pv.value);
    system.pv.max = Number.isNaN(Number(system.pv.max))
      ? 0
      : Number(system.pv.max);
    system.pc.value = Number.isNaN(Number(system.pc.value))
      ? 0
      : Number(system.pc.value);
    system.pc.max = Number.isNaN(Number(system.pc.max))
      ? 0
      : Number(system.pc.max);
  ...
  async _onDrop(event) {
    ...
    if (game.user.isGM && data.type === "Item" && data.uuid) {
      ...
      if (
        ...
        if (
          sourceItem.actor.type === "character" &&
          (sourceItem.type === "spell" || sourceItem.type === "stigma")
        ) {
          ui.notifications.warn(
            "No puedes transferir Hechizos o Estigmas de un Personaje a un NPC.",
          );
          return false;
        }
